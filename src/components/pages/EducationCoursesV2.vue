  ...
  <div>
    ...
    <b-card no-body class="mb-4">
      <b-list-group :flush="true">
        ...
        <li v-for="course in coursesData" class="list-group-item py-4">
          <div class="media flex-wrap">
            <div class="d-none d-sm-block ui-w-140">
              <a href="javascript:void(0)" class="d-block ui-rect-67 ui-bg-cover" :style="{'background-image': `url('/static/img/bg/${course.image}')`}"></a>
            </div>
            <div class="media-body ml-sm-4">
              <h5 class="mb-2">
                <div class="float-right font-weight-semibold ml-3">{{course.price ? formatPrice(course.price) : 'FREE'}}</div>
                <a href="javascript:void(0)" class="text-dark">{{course.title}}</a>
              </h5>
              ...
              <div class="mt-2">
                <span v-for="tag in course.tags" class="badge badge-outline-default text-muted font-weight-normal mr-1">{{tag}}</span>
              </div>
            </div>
          </div>
        </li>
...
export default {
  name: 'pages-education-courses-v2',
  metaInfo: {
