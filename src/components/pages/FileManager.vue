<template>
  <div>
    ...
    <div class="file-manager-container" :class="`file-manager-${viewMode}-view`">
      ...
      <div class="file-item" v-for="item in list" :class="{'selected border-primary': selected.includes(item), 'focused': focused === item || dropdownOpened === item}" @focusin="focusIn(item)" @focusout="focusOut()">
        ...
        <div class="file-item-img" :style="{'background-image': `url(${item.path})`}" v-if="isImage(item)"></div>
...
export default {
  ...
  data: () => ({
    ...
    list: [
      ...
      { type: 'file', name: 'Image-1.jpg', changed: '02/20/2018', path: '/static/img/bg/5.jpg' },
      { type: 'file', name: 'Image-2.png', changed: '02/21/2018', path: '/static/img/bg/6.jpg' },
      { type: 'file', name: 'Image-3.gif', changed: '02/22/2018', path: '/static/img/bg/7.jpg' },
