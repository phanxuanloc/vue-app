    ...
    <b-tabs class="nav-tabs-top nav-responsive-sm">
      ...
      <b-tab title="Description">
        <b-card-body>
          ...
          <quill-editor v-model="productData.description" :options="editorOptions" v-if="!isIE10" />
          <!-- Fallback -->
          <b-textarea v-model="productData.description" style="height: 600px" v-if="isIE10" />
          ...
          <div class="table-responsive">
            <b-table :items="productData.discounts" :fields="['type','code','quantity','start_date','end_date',{ key: 'actions', label: ' ' }]" class="product-item-discounts-table">
              ...
              <template slot="start_date" slot-scope="data">
                <flat-pickr v-model="data.item.start_date" :config="{ altInput: true, animate: !isRTL, dateFormat: 'm/d/Y', altFormat: 'm/d/Y' }" :placeholder="!isIE ? 'Select a start date' : null" />
              </template>
              <template slot="end_date" slot-scope="data">
                <flat-pickr v-model="data.item.end_date" :config="{ altInput: true, animate: !isRTL, dateFormat: 'm/d/Y', altFormat: 'm/d/Y', minDate: data.item.start_date }" :placeholder="!isIE ? 'Select an end date' : null" />
              </template>
        ...
        <b-card-body>
          <draggable v-model="productData.images" :options="draggableOptions">
            ...
            <div v-for="(i, index) in productData.images" :key="i.image" class="media align-items-center bg-white ui-bordered py-3 mb-2">
              ...
              <a :href="`/static/img/uikit/${i.image}`" target="_blank" class="d-block ui-w-100 mr-4">
                <img :src="`/static/img/uikit/${i.image}`" class="img-fluid" alt="">
              </a>
              <div class="media-body">
                <input type="text" :value="`example.com/static/img/uikit/${i.image}`" class="form-control form-control-sm bg-transparent border-light mb-2" readonly>
